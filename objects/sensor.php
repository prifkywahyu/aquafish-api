...
class Sensor {
    ...
    function read() {
        $query = "SELECT * FROM sensor WHERE type = ? ORDER BY created_at DESC LIMIT 0,1";
    ...
    function gets() {
        $query = "SELECT * FROM sensor WHERE type = ? ORDER BY created_at DESC LIMIT 12";
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(1, $this->type);
        $stmt->execute();
